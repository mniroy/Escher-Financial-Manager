...
import { Calendar, TrendingUp, ChevronLeft, ChevronRight, Camera, X, Plane, CreditCard, ArrowUp, ArrowDown, ChevronDown } from 'lucide-react';
...
interface DashboardProps {
  ...
  user: any; // Using any for now to avoid import issues if User type changed
}
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  return (
    <div className={`flex flex-col h-full bg-gray-50 overscroll-none ${showLogger ? 'overflow-y-auto' : 'overflow-hidden'}`}>

      {/* Hero Section with Gradient Background */}
      <div className="bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 px-4 pt-4 pb-6 relative">

        {/* Greeting & Period Selector */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <img src={user?.picture} alt="Profile" className="w-12 h-12 rounded-full border-2 border-white/30" />
            <div>
          ...
          {/* Period Selector (Integrated at top) */}
          <div className="flex items-center bg-white/20 backdrop-blur rounded-lg px-3 py-2 border border-white/10 shadow-sm transition-all hover:bg-white/30">
            <span className="text-sm font-medium text-white">{dateLabel}</span>
            <ChevronDown className="w-4 h-4 ml-1 text-white/70" />
          </div>
        </div>

        {/* Balance Card - Now "Total Balance" and "Expenses" */}
        <div className="bg-white/10 backdrop-blur rounded-2xl p-5 border border-white/10 shadow-lg">
          <div className="text-center mb-4">
            <p className="text-indigo-200 text-xs uppercase tracking-wider mb-1">Total Balance</p>
          ...
          <div className="flex justify-around pt-4 border-t border-white/10">
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-1 mb-1">
                ...
                <span className="text-[10px] uppercase text-indigo-200 font-bold">Expenses</span>
              </div>
      ...
      <div className="flex flex-col gap-3 p-3">
        ...
        {/* Stats Cards - Removed as they are now in the Hero */}
